...
\author{Kevin Pratt, Connor Riley, Don Sheehy}
\date{February 2016}
...
An edge patch is written as $<h,i;j,k>$. We say it joins the vertices $h,i$ and separates the faces $j,k$.
\begin{figure}[h]
      \centering
      ...
      \label{fig: Figure 4}
  \end{figure}\\
Def: A self stress on a framework $G(p)$ is an assignment of scalars $\omega_{ij}$ to the edges such that for each vertex $i : \sum_j \omega_{ij}(p_i - p_j) = 0$. 
A self stress is non-trivial if some $\omega_{ij} \neq 0$.
A self stress is full if all $\omega_{ij} \neq 0$.
A framework is independent if it supports only the trivial self stress with all scalars zero.
The support is the set of edges on which $\omega_{ij} \neq 0$.
The self stress is full if the support is the entire graph.

A combinatorial oriented polyhedron is a finite set of vertices, faces and edge patches such that:
\begin{enumerate}
...
Part A: We show it is vertex 2-connected, assumption 2 from above shows it is connected.
Assume the graph can be separated by removing $v_0$. 
Select $C$ surrounded by an annulus in the sphere, reinserting $v_0$. We must insert wedges from $C$ to $v_0$, around $C$ this breaks the annulus into a single face contacting $v_0$ twice. This contradicts assumption 3.
We show the graph is edge 3-connected.
Assume removing 2 edges disconnects the graph. These edges go from $\{a,a^{'}\}$ and $\{b, b^{'}\}$ joining 2 components. The two edges cut out two faces $F^1$ and $F^2$. In the cycle around $F^1$, the face $F^2$ now occurs across two edges. This contradicts 3.
...
Proof: Starting with a planar drawing of a graph, form the associated spherical polyhedron. 
...
These results generalize to any combinatorial oriented polyhedron. Given a combinatorial oriented polyhedron $(V,F,E)$ and any framework $H = ((V,E);P)$ there is a full self stress on the framework satisfying $\sum \omega_ev_e = 0$ if and only if there is a reciprocal framework $H* = ((F,E);P)$. 
