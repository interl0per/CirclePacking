...
\textsc{Lemma} Let $p \in relint(p_1,...,p_m)$ with $p, p_1,...,p_m \in \R^2$, and let $\phi$ be a linear functional on $\R^2$
	...
	\end{enumerate} 
\textsc{Proof} We have $p = \sum_{i = 1}{m} \lambda_ip_i$, with all $\lambda_i > 0$. Consider the expression $\phi(p - \sum_{i=1}^{m}\lambda_ip_i) = 0$. Linearity of $\phi$ yields $\sum_{i=1}^{m}\lambda_i\phi(p-p_i) = 0$. If the term $\phi(p-p_v)$ is negative, then there must be at least one $w \in \{\1,..,m\}$ for which $\phi(p-p_w)$ is positive.

A proof of part (ii) is left to the reader.
